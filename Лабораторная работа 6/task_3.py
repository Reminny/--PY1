...
def to_csv_file(filename: str, headers: list, rows: list[list], delimetr: str = ',', new_line: str = '\n'):
    with open(filename, 'w') as f:
        for elem in headers:
            if elem == headers[len(headers) - 1]:
                f.write(elem)
            else:
                f.write(elem + delimetr)
        f.write(new_line)
        for nested_list in rows:
            for elem in nested_list:
                if elem == nested_list[len(nested_list) - 1]:
                    f.write(elem)
                else:
                    f.write(elem + delimetr)
            f.write(new_line)
